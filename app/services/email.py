...
def send_otp_email(email: str, otp_code: str, purpose: str = "Account Verification") -> bool:
    ...
    First tries Brevo REST API over HTTPS (Cloud & Render compatible), then falls back to direct Gmail SMTP.
    """
    smtp_host = os.getenv("SMTP_HOST", "")
    smtp_port = int(os.getenv("SMTP_PORT", "587"))
    smtp_user = os.getenv("SMTP_USER", "")
    smtp_password = os.getenv("SMTP_PASSWORD", "").replace(" ", "")
    smtp_from = os.getenv("SMTP_FROM", smtp_user or "rg2216605@gmail.com")
...
"""
    ...
    # 1. Try Brevo REST API First (Instant Delivery via HTTPS Port 443 - Cloud/Render Compatible)
    if brevo_api_key:
        try:
            payload = {
                "sender": {"name": "Estateline Security", "email": smtp_from if "@" in smtp_from else "rg2216605@gmail.com"},
                "to": [{"email": email}],
            ...
            print(f"[BREVO NOTICE] Brevo API failed ({e}), attempting Gmail SMTP fallback...", flush=True)

    # 2. Try Direct Gmail SMTP Fallback
