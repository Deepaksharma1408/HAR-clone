    ...
    <div className="min-h-screen bg-bg text-ink font-inter">
      <main className="estateline-container py-12 space-y-12">
        ...
        <div className="bg-surface border border-line rounded-2xl p-8 md:p-12 shadow-sm space-y-8">
          ...
          <form onSubmit={handleCalculate} className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-bg p-6 rounded-xl border border-line">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="bg-surface border-2 border-brass/50 rounded-2xl p-8 space-y-4 shadow-md text-center">
            ...
            <div className="text-xs text-ink-soft border-t border-line pt-3">
              Estimated Value Range: <span className="font-semibold text-ink">${(calculatedValue * 0.95).toLocaleString()} – ${(calculatedValue * 1.05).toLocaleString()}</span>
            </div>
          </div>
          ...
          <div className="bg-surface border border-line rounded-2xl p-8 space-y-6 shadow-sm">
            ...
            <div className="space-y-4 text-xs font-inter">
              <div className="flex justify-between py-2 border-b border-line">
                ...
                <span className="font-semibold text-ink">${Math.round(calculatedValue / (parseFloat(sqft) || 3200))}/sqft</span>
              </div>
              <div className="flex justify-between py-2 border-b border-line">
                <span className="text-ink-soft">Estimated Monthly Rent</span>
                <span className="font-semibold text-emerald-700">${Math.round(calculatedValue * 0.007).toLocaleString()}/mo</span>
              </div>
              <div className="flex justify-between py-2 border-b border-line">
                <span className="text-ink-soft">Estimated Property Tax</span>
                <span className="font-semibold text-ink">${Math.round(calculatedValue * 0.021).toLocaleString()}/yr</span>
              </div>
              <div className="flex justify-between py-2">
                <span className="text-ink-soft">Confidence Score</span>
                <span className="font-semibold text-blue-600">94% (High Accuracy)</span>
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-left text-xs font-inter">
              <thead>
                <tr className="border-b border-line text-ink-soft">
                  ...
                  <th className="pb-3 font-medium">Beds/Baths</th>
