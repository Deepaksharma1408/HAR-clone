    ...
    <div className="min-h-screen bg-bg text-ink font-inter">
      ...
      <main className="estateline-container py-12 space-y-12">
        ...
        <div className="bg-surface border border-line rounded-[20px] p-8 md:p-12 shadow-sm space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-bg p-4 rounded-[14px] border border-line">
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-xs font-mono uppercase text-ink-soft mr-2">District:</span>
              {[
                ...
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-full text-xs font-inter font-bold transition-all cursor-pointer ${
                    district === d.id ? "bg-brass text-white shadow-sm" : "bg-surface text-ink border border-line hover:border-ink"
                  }`}
                >
                  {d.label}
                </button>
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs font-mono uppercase text-ink-soft mr-1">Level:</span>
              {["all", "Elementary", "Middle", "High"].map((lvl) => (
                <button
                  key={lvl}
                  onClick={() => setLevelFilter(lvl)}
                  className={`px-3 py-1 rounded-full text-xs font-inter font-bold transition-all cursor-pointer ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredSchools.map((s, idx) => (
            <Card key={idx} className="bg-surface p-6 rounded-[16px] border border-line flex flex-col justify-between hover:shadow-lg transition-shadow">
              <div>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <span className="text-[11px] font-mono uppercase text-brass font-bold">{s.district}</span>
                    <h3 className="font-fraunces text-xl font-bold text-ink mt-0.5">{s.name}</h3>
                  </div>
                  <div className="bg-emerald-100 text-emerald-900 border border-emerald-300 font-inter font-extrabold text-sm px-3 py-1 rounded-full">
                    {s.rating}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 bg-bg p-3 rounded-[8px] text-center border border-line text-xs font-inter">
                  <div>
                    <div className="text-[10px] font-mono uppercase text-ink-soft">Grade Level</div>
                    <div className="font-bold text-ink">{s.level}</div>
                  </div>
                  <div>
                    <div className="text-[10px] font-mono uppercase text-ink-soft">Students</div>
                    <div className="font-bold text-ink">{s.students}</div>
                  </div>
                  <div>
                    <div className="text-[10px] font-mono uppercase text-ink-soft">Student-Teacher</div>
                    <div className="font-bold text-ink">{s.ratio}</div>
                  </div>
                </div>
              </div>
