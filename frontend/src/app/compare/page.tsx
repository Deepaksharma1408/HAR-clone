  ...
  return (
    <div className="min-h-screen bg-bg text-ink font-inter">
      <main className="estateline-container py-12 space-y-10">
        {/* Header */}
        <div className="bg-surface border border-line rounded-2xl p-8 shadow-sm space-y-4">
        ...
        {loading ? (
          <div className="py-12 text-center text-xs text-ink-soft">Loading comparison matrix...</div>
        ) : properties.length === 0 ? (
          ...
          <div className="bg-surface border border-line rounded-2xl overflow-hidden shadow-md">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs font-inter border-collapse">
                <thead>
                  <tr className="bg-bg/80 border-b border-line text-ink-soft">
                    <th className="p-5 w-48 font-medium border-r border-line text-xs">Attribute / Metric</th>
                    {properties.map((p) => (
                      <th key={p.id} className="p-5 min-w-[240px] border-r border-line text-center font-normal">
                        <div className="h-32 rounded-xl overflow-hidden mb-3 bg-bg">
                          <img
                        ...
                        <span className="text-xs text-ink-soft">{p.city}, TX</span>
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-line">
                  ...
                  <tr className="hover:bg-bg/40 transition-colors">
                    <td className="p-5 font-medium text-ink border-r border-line bg-bg/20">Asking Price</td>
                    {properties.map((p) => (
                  ...
                  {/* $/sqft */}
                  <tr className="hover:bg-bg/40 transition-colors">
                    <td className="p-5 font-medium text-ink border-r border-line bg-bg/20">Price / Sq Ft</td>
                    {properties.map((p) => (
                      <td key={p.id} className="p-5 text-center font-semibold text-ink border-r border-line">
                  ...
                  {/* Bedrooms / Baths */}
                  <tr className="hover:bg-bg/40 transition-colors">
                    <td className="p-5 font-medium text-ink border-r border-line bg-bg/20">Beds / Baths</td>
                    {properties.map((p) => (
                      <td key={p.id} className="p-5 text-center text-ink-soft border-r border-line">
                        <strong className="text-ink font-semibold">{p.beds} Beds</strong> · {p.baths} Baths
                      </td>
                    ))}
                  </tr>
                  ...
                  {/* Sqft */}
                  <tr className="hover:bg-bg/40 transition-colors">
                    <td className="p-5 font-medium text-ink border-r border-line bg-bg/20">Interior Sqft</td>
                    {properties.map((p) => (
                      <td key={p.id} className="p-5 text-center text-ink-soft border-r border-line">
                  ...
                  {/* Property Type */}
                  <tr className="hover:bg-bg/40 transition-colors">
                    <td className="p-5 font-medium text-ink border-r border-line bg-bg/20">Property Type</td>
                    {properties.map((p) => (
                      <td key={p.id} className="p-5 text-center text-ink-soft border-r border-line">
                        <span className="inline-block px-3 py-1 bg-amber-100 text-amber-900 font-medium rounded-full text-xs">
                          {p.type}
                        </span>
                      </td>
                    ))}
                  ...
                  <tr className="hover:bg-bg/40 transition-colors">
                    <td className="p-5 font-medium text-ink border-r border-line bg-bg/20">Est. Monthly Mortgage</td>
                    {properties.map((p) => (
                      <td key={p.id} className="p-5 text-center font-semibold text-emerald-700 border-r border-line">
                  ...
                  <tr className="bg-bg/30">
                    <td className="p-5 font-medium text-ink border-r border-line">Full Specs</td>
                    {properties.map((p) => (
