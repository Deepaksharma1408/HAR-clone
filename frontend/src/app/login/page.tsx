      ...
      <main className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          <Card hoverable={false} className="w-full">
            ...
            {/* Error Banner */}
            {error && (
              <div className="mb-5 p-3.5 bg-danger/10 border border-danger/20 rounded-[2px]">
                <p className="text-xs text-danger font-mono uppercase tracking-wide">
                  {error}
                </p>
              </div>
            )}
            ...
            {/* Success / Info Banner */}
            {infoMessage && step === 2 && (
              <div className="mb-5 p-3.5 bg-brass/10 border border-brass/30 rounded-[2px]">
              ...
              <form onSubmit={handleVerifyLoginOTP} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="123456"
                    className="w-full bg-bg text-ink text-center tracking-[0.4em] font-mono text-2xl py-3 rounded-[2px] border border-line focus:outline-none focus:border-brass transition-colors"
                    disabled={loading}
                  />
                </div>
                ...
                <Button
                  type="submit"
                  variant="brass"
                  className="w-full py-3 mt-4 text-xs font-mono uppercase"
...
export default function LoginPage() {
  return (
    <React.Suspense fallback={<div className="min-h-screen bg-bg flex items-center justify-center text-xs font-mono">Loading Sign In...</div>}>
      <LoginFormContent />
    </React.Suspense>
  );
}
